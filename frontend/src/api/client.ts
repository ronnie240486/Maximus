...
// isso que faz a lista "Dispositivos Conectados" mostrar o canal (e o
// status online) em vez de ficar preso num valor antigo. Chamado
// periodicamente enquanto algo está tocando (ver player.tsx); se falhar
// (sem internet, painel fora do ar etc.), simplesmente não atualiza dessa
// vez — nunca interrompe a reprodução.
//
...
// rencia_app). A rota `/api/v4/heartbeat.php?...&content=...` que eu tinha
// usado antes EXISTE e responde 200, mas é só leitura — nunca grava nada,
// o que enganou por um tempo. Testei a rota certa direto no servidor e
// confirmei `"contentUpdated":true` na resposta.
async function heartbeatAt(base: string, mac: string, content: string): Promise<boolean> {
  try {
    const res = await fetch(
      proxied(`${base}/heartbeat?mac=${encodeURIComponent(mac)}&current_content=${encodeURIComponent(content)}`),
...
export async function sendHeartbeat(mac: string, content: string): Promise<void> {
  if (await heartbeatAt(PANEL_BASE, mac, content)) return;
  await heartbeatAt(PANEL_BASE_FALLBACK, mac, content);
}
