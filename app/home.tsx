...
export default function HomeScreen() {
    ...
    React.useCallback(() => {
      ...
      (async () => {
        ...
        loadWatchHistory().then((history) => {
          const safeHistory = filterOutAdultTitles(history);
          setContinueWatching(
            safeHistory.length
              ? {
                  title: 'CONTINUE ASSISTINDO',
                  items: safeHistory.slice(0, 15).map((h) => ({
                    id: `continue-${h.id}`,
                    name: h.name,
                    logo: h.logo,
                    stream: h.stream,
  ...
  const load = useCallback(async () => {
    ...
    if (cache?.sections) {
      ...
      setSlots((prev) => ({
        ...prev,
        live: cachedSections.live
          ? { ...cachedSections.live, items: filterOutAdultTitles(cachedSections.live.items) }
          : undefined,
        movies: cachedSections.movies
          ? { ...cachedSections.movies, items: filterOutAdultTitles(cachedSections.movies.items) }
          : undefined,
        series: cachedSections.series
          ? { ...cachedSections.series, items: filterOutAdultTitles(cachedSections.series.items) }
          : undefined,
      }));
      setLoading(false);
    }
    ...
    const liveP = Promise.all([xtream.liveStreams(creds), liveCatsP]).then(([live, liveCats]) => {
      const nonAdultLive = filterOutAdultTitles(filterOutAdultItems(live || [], liveCats || []));
      const filteredLive = kidsMode
        ? filterToKidsItems(nonAdultLive, liveCats || [])
        : nonAdultLive;
      ...
      if (gotSomething) {
        ...
        loadListCache<XtreamCategory, XtreamLive>('channels').then((existing) => {
          saveListCache(
            'channels',
            filterOutAdultCategories(existing?.categories || []),
            filteredLive || [],
          );
        });
      }
    ...
    const moviesP = Promise.all([xtream.vodStreams(creds), vodCatsP]).then(([movies, vodCats]) => {
      const nonAdultMovies = filterOutAdultTitles(filterOutAdultItems(movies || [], vodCats || []));
      const filteredMovies = kidsMode
        ? filterToKidsItems(nonAdultMovies, vodCats || [])
        : nonAdultMovies;
      ...
      if (gotSomething) {
        ...
        loadListCache<XtreamCategory, XtreamMovie>('movies').then((existing) => {
          saveListCache(
            'movies',
            filterOutAdultCategories(existing?.categories || []),
            filteredMovies || [],
    ...
    const seriesP = Promise.all([xtream.seriesList(creds), seriesCatsP]).then(([series, seriesCats]) => {
      const nonAdultSeries = filterOutAdultTitles(filterOutAdultItems(series || [], seriesCats || []));
      const filteredSeries = kidsMode
        ? filterToKidsItems(nonAdultSeries, seriesCats || [])
        : nonAdultSeries;
      ...
      if (gotSomething) {
        ...
        loadListCache<XtreamCategory, XtreamSeries>('series').then((existing) => {
          saveListCache(
            'series',
            filterOutAdultCategories(existing?.categories || []),
            filteredSeries || [],
      ...
      (async () => {
        try {
          ...
          if (movieCats || movieStreams) {
            await saveListCache(
              'movies',
              filterOutAdultCategories(movieCats || []),
              filterOutAdultTitles(filterOutAdultItems(movieStreams || [], movieCats || [])),
            );
          }
        } catch {}
        try {
          ...
          if (seriesCats || seriesStreams) {
            await saveListCache(
              'series',
              filterOutAdultCategories(seriesCats || []),
              filterOutAdultTitles(filterOutAdultItems(seriesStreams || [], seriesCats || [])),
            );
          }
        } catch {}
      })();
