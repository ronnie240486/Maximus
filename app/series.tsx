...
export default function SeriesScreen() {
  ...
  const load = useCallback(async () => {
    ...
    if (cache) {
      const safeCategories = filterOutAdultCategories(cache.categories);
      const safeSeries = filterOutAdultTitles(filterOutAdultItems(cache.items, cache.categories));
    ...
    const catsPromise = xtream.seriesCategories(creds);
    ...
    if (safeList.length) setSeries(safeList);
    setLoading(false);
  ...
  const visibleSeries = useMemo(
    () => {
      const safe = filterOutAdultTitles(filterOutAdultItems(series, categories));
  ...
  const openSeries = (s: XtreamSeries) => {
    ...
    if (isAdultTitle(s.name) || isAdultCategoryName(categoryName)) return;
    guard(categoryName, () => {
      router.push({
