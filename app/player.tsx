...
import { isAdultCategoryName, isAdultTitle, filterOutAdultCategories, filterOutAdultItems, filterOutAdultTitles } from '@/src/lib/adult-content';
...
export default function PlayerScreen() {
  ...
  const openChannelGrid = useCallback(async () => {
    ...
    setChannelList(safeList);
    setCategories(safeCats);
    setLoadingChannels(false);
  }, [channelList.length]);

  const switchChannel = useCallback((s: XtreamLive) => {
    if (isAdultTitle(s.name)) return;
    const creds = getXtream();
    if (!creds) return;
  ...
  const isAdult = params.adult === '1' || isAdultTitle(params.name);
  ...
  }, [current.stream, isAdult, isLive, sessionKind, sessionSource, setSource]);
