...
export default function SeriesDetailsScreen() {
  ...
  const blocked = params.adult === '1' || isAdultTitle(params.name);
  ...
  const playEpisode = (ep: XtreamEpisode) => {
    if (blocked || isAdultTitle(info?.info.name || params.name) || isAdultTitle(ep.title)) return;
    const creds = getXtream();
    if (!creds) return;
    router.push({
      pathname: '/player',
    ...
    return (
      <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
        <View style={styles.header}>
          <Pressable onPress={() => router.back()} hitSlop={16} style={styles.backBtn} testID="sd-back-blocked">
            <Ionicons name="chevron-back" size={24} color={colors.white} />
          </Pressable>
          <Text style={styles.headerTitle}>Conteúdo indisponível</Text>
          <View style={{ width: 24 }} />
        </View>
        <View style={styles.center}>
          <MaterialCommunityIcons name="shield-lock" size={48} color={colors.accentCyan} />
          <Text style={styles.errorSub}>Este conteúdo não está disponível.</Text>
        </View>
      </SafeAreaView>
    );
  }
