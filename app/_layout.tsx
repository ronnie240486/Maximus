...
import { filterOutAdultItems, filterOutAdultTitles } from '@/src/lib/adult-content';
  ...
  useEffect(() => {
    ...
    const openStream = async (streamId: number | undefined) => {
      ...
      try {
        ...
        const safeStreams = filterOutAdultTitles(filterOutAdultItems(streams || [], categories || []));
        const ch = safeStreams.find((s) => s.stream_id === streamId);
        if (!ch) return;
        router.push({
          pathname: '/player',
          params: {
