...
export default function RecommendScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const safeMovieCats = filterOutAdultCategories(movieCats);
    const safeMovieItems = filterOutAdultTitles(filterOutAdultItems(movieItems, movieCats));
    const safeSeriesCats = filterOutAdultCategories(seriesCats);
    const safeSeriesItems = filterOutAdultTitles(filterOutAdultItems(seriesItems, seriesCats));
    setMoviePool({ items: safeMovieItems, categories: safeMovieCats });
    setSeriesPool({ items: safeSeriesItems, categories: safeSeriesCats });
    setLoading(false);
  }, []);
  ...
  const pick = useCallback(() => {
    ...
    const movieCats = kidsMode ? filterToKidsCategories(moviePool.categories) : filterOutAdultCategories(moviePool.categories);
    const movieItemsSafe = filterOutAdultTitles(filterOutAdultItems(moviePool.items, moviePool.categories));
    const movieItems = kidsMode ? filterToKidsItems(movieItemsSafe, moviePool.categories) : movieItemsSafe;
    const seriesCats = kidsMode ? filterToKidsCategories(seriesPool.categories) : filterOutAdultCategories(seriesPool.categories);
    const seriesItemsSafe = filterOutAdultTitles(filterOutAdultItems(seriesPool.items, seriesPool.categories));
    const seriesItemsBase = kidsMode ? filterToKidsItems(seriesItemsSafe, seriesPool.categories) : seriesItemsSafe;
  ...
  const openItem = (item: SuggestionItem) => {
    ...
    if (isAdultTitle(item.name) || isAdultCategoryName(categoryName)) return;
    guard(categoryName, () => {
      if (item.kind === 'movie') {
