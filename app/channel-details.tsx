...
export default function ChannelDetailsScreen() {
  ...
  const blockedDirectly = params.adult === '1' || isAdultTitle(params.name) || isAdultCategoryName(params.categoryName);

  useEffect(() => {
    if (blockedDirectly) router.back();
  }, [blockedDirectly, router]);
  ...
  useEffect(() => {
    if (blockedDirectly) return;
    setSource(initialStreamUrl || null, true, 'live').catch(() => setVideoError(true));
  }, [blockedDirectly, initialStreamUrl, setSource]);
  ...
  const loadRelated = useCallback(async () => {
    ...
    if (cache) {
      const safeCats = filterOutAdultCategories(cache.categories);
      const safeItems = filterOutAdultTitles(filterOutAdultItems(cache.items, cache.categories));
      applyRelated(safeItems, safeCats);
    }

    const [liveList, cats] = await Promise.all([xtream.liveStreams(creds), xtream.liveCategories(creds)]);
    if (liveList && cats) {
      const safeCats = filterOutAdultCategories(cats);
      const safeItems = filterOutAdultTitles(filterOutAdultItems(liveList, cats));
      applyRelated(safeItems, safeCats);
      saveListCache('channels', safeCats, safeItems);
    }
  }, [creds, applyRelated]);
  ...
  const openChannelGrid = useCallback(async () => {
    ...
    setChannelList(filterOutAdultTitles(filterOutAdultItems(list || [], cats || [])));
    setCategories(filterOutAdultCategories(cats || []));
    setLoadingChannels(false);
  }, [channelList.length, creds]);

  const switchToChannel = useCallback((c: XtreamLive) => {
    if (isAdultTitle(c.name)) return;
  ...
  const openFullscreenPlayer = () => {
    if (!creds || blockedDirectly || isAdultTitle(current.name) || isAdultCategoryName(resolvedCategoryName)) return;
    enterFullscreen();
    router.push({
      pathname: '/player',
      params: {
