...
export default function SearchScreen() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // O bloqueio global vale para todos os perfis na busca e no comando de voz.
      const safeLive = filterOutAdultTitles(filterOutAdultItems(l || [], liveCats || []));
      const safeMovies = filterOutAdultTitles(filterOutAdultItems(m || [], vodCats || []));
      const safeSeries = filterOutAdultTitles(filterOutAdultItems(s || [], seriesCats || []));
      setLive(kidsMode && liveCats ? filterToKidsItems(safeLive, liveCats) : safeLive);
      setMovies(kidsMode && vodCats ? filterToKidsItems(safeMovies, vodCats) : safeMovies);
      setSeries(kidsMode && seriesCats ? filterToKidsItems(safeSeries, seriesCats) : safeSeries);
      setLoading(false);
      if (!params.voice) {
