...
export default function MoviesScreen() {
  ...
  const load = useCallback(async () => {
    ...
    if (cache) {
      ...
      const safeMovies = filterOutAdultTitles(filterOutAdultItems(cache.items, cache.categories));
    ...
    const catsPromise = xtream.vodCategories(creds);
    ...
    if (safeList.length) setMovies(safeList);
    setLoading(false);
  ...
  const visibleMovies = useMemo(
    () => {
      const safe = filterOutAdultTitles(filterOutAdultItems(movies, categories));
  ...
  const openMovie = (m: XtreamMovie) => {
    ...
    if (isAdultTitle(m.name) || isAdultCategoryName(categoryName)) return;
    guard(categoryName, () => {
      router.push({
