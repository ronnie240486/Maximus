...
export default function ChannelsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    if (cache) {
      const safeCacheCategories = filterOutAdultCategories(cache.categories);
      const safeCacheItems = filterOutAdultTitles(filterOutAdultItems(cache.items, cache.categories));
      setCategories(safeCacheCategories);
      setStreams(safeCacheItems);
      setLoading(false);
    }
    ...
    const catsPromise = xtream.liveCategories(creds);
    const list = await xtream.liveStreams(creds);
    const cats = await catsPromise;
    const safeCategories = filterOutAdultCategories(cats || []);
    const safeList = filterOutAdultTitles(filterOutAdultItems(list || [], cats || []));
    if (safeCategories.length) setCategories(safeCategories);
    if (safeList.length) {
      setStreams(safeList);
      setLoadError(null);
    } else if (!cache) {
    ...
    if (safeList.length) {
      saveListCache(CACHE_KEY, safeCategories, safeList);
      ...
      if (!isLowEndDevice) {
        Promise.all(safeList.slice(0, 15).map((c) => xtream.shortEpg(creds, c.stream_id, 1).catch(() => null)));
      }
    }
  }, [isLowEndDevice]);
  ...
  const visibleCategories = useMemo(
    () => (kidsMode ? filterToKidsCategories(categories) : filterOutAdultCategories(categories)),
    [categories, kidsMode]
  );
  ...
  const visibleStreams = useMemo(
    () => {
      const safe = filterOutAdultTitles(filterOutAdultItems(streams, categories));
      return kidsMode ? filterToKidsItems(safe, categories) : safe;
    },
